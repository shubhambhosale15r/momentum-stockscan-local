...
def initialize_session_state():
    for key in ['view_universe_rankings', 'view_recommended_stocks', 'analyze_button_clicked', 'view_high_momentum']:
...
st.markdown(f"""
    ...
    <div style="text-align: center; font-size: 1.1rem; color: #aaa;">
       Select a sector to analyze top momentum stocks. Also view overall sector rankings, top picks, or high momentum stocks.
    </div><br>
""", unsafe_allow_html=True)
...
# Add new helper to get all tickers
ALL_TICKERS = [t for sub in SECTOR_UNIVERSE.values() for t in sub]
...
def create_sidebar():
    with st.sidebar:
        ...
        if st.button("High Momentum Stocks"): st.session_state.view_high_momentum = True
    return sector, tickers

sector_universe_name, universe_tickers = create_sidebar()

# unchanged: read_local_stock_data, calculate_returns, analyze_universe...
...
def analyze_universe(universe_name, universe_tickers):
    ...
    for ticker in universe_tickers:
        ...
        df_weekly['Weekly Return'] = df_weekly['Close'].pct_change(fill_method='pad')
...
# New: get top 10 across all tickers irrespective of universe

def get_top_momentum_stocks_all(n=10):
    df, _ = analyze_universe('All', ALL_TICKERS)
    if df.empty: return df
    return df.sort_values("Momentum Score", ascending=False).head(n)
...
def main():
    ...
    if st.session_state.view_high_momentum:
        st.subheader("High Momentum Stocks: Top 10 Overall")
        loading = st.empty()
        loading.markdown(f"<div class='loading-container'><p class='loading-text'>{LOADING_TEXT}</p></div>", unsafe_allow_html=True)
        try:
            top_all = get_top_momentum_stocks_all(10)
            loading.empty()
            if not top_all.empty:
                st.dataframe(top_all.style.format({
                    "12-Month Return (%)": "{:.2f}%",
                    "6-Month Return (%)": "{:.2f}%",
                    "3-Month Return (%)": "{:.2f}%",
                    "Annualized Volatility": "{:.4f}",
                ...
                st.warning("No data available to compute high momentum stocks.")
        except Exception as e:
            st.error(f"Error: {e}")
        finally:
